...
#
# 用法（在 nightjar 项目根目录）：
#   ./scripts/doctor.sh                                  # 目标容器自动发现
#   ./scripts/doctor.sh --targets app-mysql,app-redis    # 指定要检查的目标容器
#   ./scripts/doctor.sh --target-container app-backend   # 指定日志采集目标容器
#
...
# 目标容器：留空自动发现（排除平台自身 mwops-*），不绑定任何具体项目。
TARGETS=()
TARGET_CONTAINER=""

# discover_targets 未显式指定时，列出这台机器上"看起来该被监控"的容器。
discover_targets() {
...
if [ -n "$sock_src" ]; then
  ...
  case "$docker_flag" in
    ...
    *) bad "INTEGRATION_DOCKER_ENABLED=${docker_flag:-未设置}：一键拉起 Exporter/代建账号/日志接入都会不可用"
       hint "在 $NJ_ENV 里设为 true（或重跑 ./scripts/onboard.sh）" ;;
  esac
else
  ...
  hint '或直接重跑 ./scripts/onboard.sh（它会自动放开注释）'
fi
