...
2. `dial redis: unknown network redis` 是 Go `net.Dial(network, addr)` 的报错格式：
   网络类型被当成了 `redis`。也就是说，地址里的 **scheme 被当成了网络名**；
3. 平台注入的是 `REDIS_ADDR=redis://127.0.0.1:6379`（`Address.URL("redis")`）；
   redis_exporter 的 README 虽写明 `redis://host:6379` 与 `redis.example.com:6379` 都合法，
   但 v1.66 在**单实例 `/metrics` 路径**上把 scheme 直接当网络类型用；
4. 用一个不依赖平台、也不改容器的办法当场确认：
   `curl 'http://127.0.0.1:9121/scrape?target=127.0.0.1:6379'` —— 同一份口令、
   同一个 Exporter，只换地址写法。

**根因**

平台按"URL 更规范"的直觉选了带 scheme 的写法，而 Exporter 的这一条代码路径并不接受它。
两边都符合各自的文档，但合起来就是不工作——**平台侧只能看到 `up=0`，极易被归因成
"Redis 没起来 / 地址填错 / 口令不对"**，排查方向完全跑偏。

**修复**

1. `redisAddr()`：默认注入**不带 scheme** 的 `host:port`；
   只有当用户显式带路径（如 `redis://host:6379/2` 指定 db）时才保留 URL 形态，避免丢语义；
2. 模板 Notes 与 `docs/INTEGRATION.md` §8 排查表写明"不要手工改成 `redis://…`"及现象特征；
3. 现有集成点一次「重新应用」即可刷新 env 文件（无需重建容器以外的任何东西）。
...
3. 原 `TestRenderRedisIntegration` 断言的正是旧写法（`REDIS_ADDR: redis://…`）——
...
15. **"两边都符合各自文档"也会不工作**：平台按 URL 规范写 `redis://…`，Exporter 按自己的
    ...
    **能直接验证的那一侧就用最小实验验证**（`/scrape?target=host:port` 一条命令，
    不重建、不改容器），比读两边文档猜快得多；确认后再把结论固化进模板与测试。
