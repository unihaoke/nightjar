...
| 规则匹配 | 按「服务 + 日志消息原文 + 级别」匹配 `log_alert_rules`：**priority 数字小的优先，同优先级按 id 升序，取第一条命中的启用规则**（`MatchLogAlertRule`） |
| 命中不到规则 | **不产生告警**：不入库、不通知、不分析（`matchRule` 不回落任何默认规则）——告警只能来自页面上新增过的规则 |
...
| `signature_pattern` | 空 = 任意消息 | 匹配**上报的 `message` 原文**（不是错误指纹）：**普通文本 = 子串匹配**（大小写不敏感）；`/正则/` 形式 = 正则匹配（大小写不敏感；正则写错会退化为子串匹配，不会让链路挂掉） |
...
> **`signature_pattern` 比的是日志消息原文**，填日志里那句话的一段即可（如
> `NullPointerException`、`Request method 'GET' is not supported`）。
> 它**不是**错误指纹：指纹是归一化后的哈希（`ErrorSignature`），人写不出来，拿哈希去配必然永不命中。
> 最省事的做法：**先不配 `signature_pattern`（留空 = 任意消息），只按服务名与级别过滤**，
> 等事件进来后照着详情页的「错误消息」填一段。
...
② 按事件的服务名查「代码仓库」映射（CodeRepo.FindByService）
      ...
      ▼
③ 拉代码：**本地没有代码就 clone，有代码就更新到远端**（internal/repo，见下）
      ▼
...
**缓存行为（`internal/repo`）**——判定只有一句话：**本地有没有可用的代码**。
有 → 更新到远端（pull / fetch + checkout）；没有 → clone。判定的"没有"含三种形态：
目录不存在、目录是空的、`.git` 是残缺的（clone 被中断留下的半成品）。
只有"目录里有内容又不是 git 仓库"才拒绝覆盖（那可能是别人的目录）。

| 拉取时机 | 行为 |
|---|---|
| 平台启动（容器重新构建后） | 异步预热所有已配置的映射：没有就 clone，有就 pull。失败只记日志、不阻断启动，失败的仓库在事件处理里重试 |
| 事件分析（worker） | 先确保本地有代码，再交给 AI 定位 |
| 手工分析 / 事件重试 | 分析入口自己兜一次：本地没有就 clone，有就直接用（不重复 pull） |
...
| 最小拉取间隔 | `code_repo.refresh_interval_seconds`（默认 300s）：一次故障风暴里同服务的多条事件不会每条都去 pull 远端。**只在本地确实有代码时生效**——代码没了（卷被重建/目录被清）就必须重新 clone，不能靠"刚拉过"跳过 |
...
| 目标目录已存在、有内容但不是 git 仓库 | **明确报错、绝不覆盖**（可能是别人的目录） |
| 目标目录是空的 / `.git` 残缺 | 视为「上次 clone 被中断」，删除后**重新 clone**（留着只会让该服务永远拿不到代码） |
