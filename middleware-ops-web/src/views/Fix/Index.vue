...
async function load(): Promise<void> {
  loading.value = true
  try {
    const [options, list, fixOptions] = await Promise.all([
      middlewareApi.list({ page: 1, page_size: 100 }),
      fixApi.history({ page: query.page, page_size: query.page_size }),
      fixApi.options(),
    ])
    instances.value = options.list || []
    history.value = list.list || []
