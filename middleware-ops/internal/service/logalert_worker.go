...
// ident 是回调报文里的三个号（幂等键 / runId / taskId），由解析层原样带出：
// 对号用哪个由 CompleteByTask 决定，这里不做取舍，避免诊断信息在半路被丢掉。
func (w *LogAlertWorker) CompleteTask(ctx context.Context, ident CallbackIdentity, status, answer, errMsg string) error {
	if w == nil || w.analysis == nil {
		return apperr.New(apperr.CodeInternal, "AI 分析能力未装配")
	}
	completion, err := w.analysis.CompleteByTask(ctx, ident, status, answer, errMsg)
	if err != nil {
		return err
	}
