...
type LogInput struct {
	...
	InstallMode string
	// Overwrite 表示是否"覆盖 Filebeat"（模板参数 MWOPS_LOG_OVERWRITE）。
	//
	// 语义刻意限定在**安装/拉取**这一层（不在配置层，见下）：
	//   - false（默认，幂等）：目标机上已有 Filebeat 就复用——不重新下载安装包；
	//     docker 模式下本地已有该镜像也不重新拉取。首次缺失时照常安装；
	//   - true（覆盖）：忽略"已存在"，重新拉取安装包/镜像并覆盖安装，
	//     用于升级版本、或修复被改坏/装了一半的 Filebeat。
	//
	// 为什么**不**把配置文件也纳入这个开关：filebeat.yml 是平台自己的产物，
	// 它的内容由平台配置推导（例如 Kafka 对外地址改了就必须重新下发）。若"不勾选就不覆盖配置"，
	// 使用者会遇到"平台改了地址、界面一切正常、目标机还在用旧配置"——正是 INC-016/INC-028
	// 反复出现的那类"看起来成功了但链路是断的"。因此配置**始终按内容同步**（内容没变不重启），
	// 这个开关只决定"要不要动目标机上那个 Filebeat 程序本体"。
	Overwrite bool
}
