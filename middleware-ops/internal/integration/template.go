...
var templates = map[string]Template{
	TypeRedis: {
		...
		Alerts: []AlertTemplate{
			{Name: "Redis 服务不可用", MetricName: "redis_up", Operator: "<", Threshold: 1,
				Level: "critical", TimeWindow: 2, Cooldown: 10, Description: "Exporter 连不上 Redis（地址/口令/网络），或进程已退出"},
			...
			{Name: "Redis 主从链路断开", MetricName: "master_link_up", Operator: "<", Threshold: 1,
				Level: "critical", TimeWindow: 2, Cooldown: 10, Description: "从库与主库的复制链路中断（非从库实例不会有该指标，不会误报）"},
		},
	...
	TypeMySQL: {
		...
		Alerts: []AlertTemplate{
			{Name: "MySQL 服务不可用", MetricName: "mysql_up", Operator: "<", Threshold: 1,
				Level: "critical", TimeWindow: 2, Cooldown: 10, Description: "Exporter 连不上 MySQL（地址/账号/网络），或实例已退出"},
			...
			{Name: "MySQL 连接使用率过高", MetricName: "connection_usage_percent", Operator: ">", Threshold: 85,
				Level: "warning", TimeWindow: 5, Cooldown: 10, Description: "已用连接占 max_connections 的比例过高（按实例自身上限判断更准）"},
